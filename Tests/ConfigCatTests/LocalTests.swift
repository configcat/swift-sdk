...
class LocalTests: XCTestCase {
    ...
    func testLocalOverRemote() throws {
        ...
        let client = ConfigCatClient(sdkKey: "testKey", pollingMode: PollingModes.autoPoll(), logger: NoLogger(), httpEngine: engine, hooks: Hooks(), flagOverrides: LocalDictionaryDataSource(source: dictionary, behaviour: .localOverRemote))
        let expectation = self.expectation(description: "wait for response")
        client.getAllValues { values in
    ...
    func testRemoteOverLocal() throws {
        ...
        let client = ConfigCatClient(sdkKey: "testKey", pollingMode: PollingModes.autoPoll(), logger: NoLogger(), httpEngine: engine, hooks: Hooks(), flagOverrides: LocalDictionaryDataSource(source: dictionary, behaviour: .remoteOverLocal))
        let expectation = self.expectation(description: "wait for response")
        client.getAllValues { values in
